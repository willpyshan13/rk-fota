...
public class MiYuePackageDownloadActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mRemoteURI = intent.getStringExtra("uri");
        mOtaPackageVersion = intent.getStringExtra("OtaPackageVersion");
        mSystemVersion = intent.getStringExtra("SystemVersion");
        mOtaPackageName = intent.getStringExtra("OtaPackageName");
        mOtaPackageLength = intent.getStringExtra("OtaPackageLength");
        mOtaVersionId = intent.getStringExtra("OtaVersionId");
        
        if(mRemoteURI == null) {
        	return;
        }
        
        setContentView(R.layout.miyue_package_download);
        setFinishOnTouchOutside(false);
        	        
        if(mRemoteURI.startsWith("ftp://")) {
        	mDownloadProtocol = DOWNLOAD_PROTOCOL_FTP;
        	mFTPRequest = parseFtpUri(mRemoteURI);
        }else {
        	mDownloadProtocol = DOWNLOAD_PROTOCOL_HTTP;
        	try {
				mHttpUri = new URI(mRemoteURI);
      ...
      mVersionId = intent.getStringExtra("OtaVersionId");
        ...
        if(packageSize_string != null)
        	 mTxtState.setText(getString(R.string.ota_update) + mOtaPackageVersion + getString(R.string.ota_package_size) + packageSize_string);
        else
        	 mTxtState.setText(getString(R.string.ota_update) + mOtaPackageVersion) ;
